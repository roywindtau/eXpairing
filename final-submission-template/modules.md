...
1. **Personalized Wine Feed (`GET /wine/ranked`)**: Applies hard style filters and checks user rating counts. *Cold start* (0 ratings) → Bayesian popularity prior. *Warming* (1-4 ratings) → `0.7·CB + 0.3·popularity` over a content-based taste profile. *Warm* (≥5 ratings) → `0.45·ALS_CF + 0.45·CB + 0.10·popularity` min-max calibrated. Active app users are folded in dynamically by solving online ALS user updates (`C = 1 + 5·rating`) against frozen item factors. In every state the top 3xTop_N pool is MMR-reranked (MMR $\lambda = 0.7$) over pairwise content-vector cosine.
