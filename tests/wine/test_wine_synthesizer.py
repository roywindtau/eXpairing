...
from backend.services.wine import synthesizer as wine_synthesizer
...
def db_session(monkeypatch):
    ...
    monkeypatch.setattr(serve_cb, "model_available", fake_model_available)
    monkeypatch.setattr(serve_cb, "cb_for_recipe",   fake_cb_for_recipe)

    # Re-enable kill switch every test (sometimes flipped off in previous test).
    monkeypatch.setattr(wine_synthesizer, "ENABLE_SYNTHETIC_WINE_RATINGS", True)
...
def test_low_rating_writes_nothing(db_session):
    n = wine_synthesizer.maybe_synthesize_on_recipe_rating(
...
def test_high_rating_writes_events(db_session):
    ...
    assert n > 0
    events = db_session.query(WineEvent).all()
    assert len(events) == n
    assert all(e.synthetic is True for e in events)
    assert all(e.rating == wine_synthesizer.SYNTHETIC_RATING for e in events)
...
def test_kill_switch_disables_synthesis(db_session, monkeypatch):
    monkeypatch.setattr(wine_synthesizer, "ENABLE_SYNTHETIC_WINE_RATINGS", False)
    n = wine_synthesizer.maybe_synthesize_on_recipe_rating(
...
def test_beef_recipe_picks_beef_wine(db_session):
    ...
    wine_synthesizer.maybe_synthesize_on_recipe_rating(
        user_id=42, recipe_id=1001, rating=5.0, db=db_session
    )
    wine_event_ids = {
        e.wine_id for e in db_session.query(WineEvent).all() if e.wine_id < 100
    }
...
def test_caps_at_n_per_kind(db_session):
    """No more than N_SYNTHETIC events per kind."""
    wine_synthesizer.maybe_synthesize_on_recipe_rating(
        user_id=42, recipe_id=1001, rating=5.0, db=db_session
    )
    n_wine = db_session.query(WineEvent).filter(WineEvent.wine_id < 100).count()
    assert n_wine <= wine_synthesizer.N_SYNTHETIC
...
def test_does_not_overwrite_explicit_rating(db_session):
    ...
    db_session.add(WineEvent(user_id=42, wine_id=1, event_type="rate",
...
def test_does_not_double_synthesize_same_pair(db_session):
    ...
    n_first = db_session.query(WineEvent).count()

    wine_synthesizer.maybe_synthesize_on_recipe_rating(
        user_id=42, recipe_id=1001, rating=5.0, db=db_session
    )
    n_second = db_session.query(WineEvent).count()

    assert n_first == n_second  # nothing new added


def test_different_users_get_independent_history(db_session):
    wine_synthesizer.maybe_synthesize_on_recipe_rating(42, 1001, 5.0, db_session)
    wine_synthesizer.maybe_synthesize_on_recipe_rating(43, 1001, 5.0, db_session)

    n_42 = db_session.query(WineEvent).filter(WineEvent.user_id == 42).count()
    n_43 = db_session.query(WineEvent).filter(WineEvent.user_id == 43).count()
...
def test_synthesizer_swallows_exceptions(db_session, monkeypatch):
    ...
    monkeypatch.setattr(serve_cb, "cb_for_recipe", crash)
...
def test_falls_back_to_popularity_when_cb_unavailable(db_session, monkeypatch):
    ...
    monkeypatch.setattr(serve_cb, "model_available", lambda: False)
