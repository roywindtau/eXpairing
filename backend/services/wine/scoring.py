...
scoring.py
----------
Final ranking layer for wines. Combines CB, CF, expert, and prior signals
into a single ranked list of WineScore objects.
...
This keeps scoring pure-functional and trivial to unit test with
...
class WineScore:
    """
    Full scoring breakdown for one wine against a recipe (Path A) or
...
def rank_wines_for_recipe(
    ...
    Args:
        ...
        candidates:    iterable of Wine ORM rows
    ...
    Returns:
        list[WineScore] sorted by final_score descending.
    """
    candidates = list(candidates)
    if not candidates:
        return []

    scores: list[WineScore] = []
    for d in candidates:
        scores.append(WineScore(
            wine_id      = d.id,
            wine_name    = d.name,
