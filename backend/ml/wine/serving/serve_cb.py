...
def _scores_from_vector(query_vec) -> dict[int, float]:
    """Cosine `query_vec` (1 x vocab) against the full wine matrix."""
...
def cb_for_recipe(recipe) -> dict[int, float]:
    """
    Path A: cosine similarity between this recipe and every wine.

    Args:
        recipe:       any object with .ingredients_csv and .tags_csv attrs
                      (Recipe ORM row or SimpleNamespace in tests)
    Returns:
        dict[wine_id, cosine in [0,1]]   (empty if model not loaded)
    """
    _load()
    ...
    return _scores_from_vector(query_vec)
...
def cb_for_user(
    ...
    return _scores_from_vector(profile_vec)
