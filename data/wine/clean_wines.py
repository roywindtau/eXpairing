...
Reads raw CSVs from data/wine/ and writes two clean CSVs ready
for seeding:
    data/wine/clean_wines.csv
    data/wine/clean_ratings.csv
...
def validate(wines: pd.DataFrame, ratings: pd.DataFrame) -> None:
    ...
    assert ratings["wine_id"].notna().all(), "Null wine_ids in ratings"

    # Every rated wine must exist in the catalog
    orphan_wines = set(ratings["wine_id"].unique()) - set(wines["id"].unique())
